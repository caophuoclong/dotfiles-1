...
#(YTFZF_EXTMENU)
external_menu="rofi -dmenu -i -l 30 -p Search:"
...
send_select_video_notif() {
  ...
  defafult_icon=/home/vu/.config/dunst/icons/play.svg
  #if downloading, say Downloading not currently playing
  if [ $is_download -eq 1 ]; then
    dunstify "Downloading" "$video_title" -a "Youtube Music" -i "$video_thumb"
  else
    dunstify "$video_title" -a "Youtube Music" -i "$video_thumb"
  fi
